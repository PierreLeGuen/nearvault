// Originally comes from: https://github.com/near/near-api-js/blob/b44b2656b7ef66fe7682243ef4838766364a9628/packages/accounts/src/account_multisig.ts#L7
...
import { Account, type Connection } from "near-api-js";
...
export class AccountMultisig extends Account {
  public storage: any;
  public onAddRequestResult: (a: any) => any;
  ...
  async signAndSendTransaction({
    ...
    try {
      result = await super.signAndSendTransaction({
        ...
        actions: [
          functionCall("add_request", args, MULTISIG_GAS, MULTISIG_DEPOSIT),
        ],
      });
    } catch (e) {
      console.error(e);
  ...
  async checkMultisigCodeAndStateStatus(contractBytes?: Uint8Array): Promise<{
    ...
    try {
      ...
      if (
        new RegExp(
          MultisigDeleteRequestRejectionError.CANNOT_DESERIALIZE_STATE
        ...
        new RegExp(
          MultisigDeleteRequestRejectionError.MULTISIG_NOT_INITIALIZED
        ).test(e && e.kind && e.kind.ExecutionError)
      ) {
        return {
          codeStatus: validCodeStatusIfNoDeploy,
        ...
        new RegExp(MultisigDeleteRequestRejectionError.METHOD_NOT_FOUND).test(
          e && e.message
        )
      ) {
